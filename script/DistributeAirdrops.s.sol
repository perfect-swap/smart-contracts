...
contract DistributeAirdrops is Script {
    ...
    constructor() {
        ...
        PRFCT = Prfct(abi.decode(jsonConstants.parseRaw(".PRFCT"), (address)));
    ...
    function run() public {
        ...
        uint256 prfctBal = PRFCT.balanceOf(address(airdrop));
        if (sum > prfctBal) amounts[0] = amounts[0] - (sum - prfctBal); // remove dust from first airdrop if needed
